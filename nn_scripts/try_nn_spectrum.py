...
sys.path.append("..")
from parse_data import parse_single_file, shuffle_and_split, generate_invd_list
...
def extract_number_of_atoms(file_path, lines_to_skip):
...
tgt = "site" # 'cpl' for coupling or 'site' for site energy
# linestotal = 2314144 # lines + withou last empty line
...
hp_maxepochs = 40 # for tuner object 
...
# david Logger so that output will be written to both terminal and stdout
class Logger(object):
    def __init__(self):
...
if tgt=="site":
    ...
    xyz_data, couplings = parse_single_file(traindata, 2*natoms)
...
if tgt=="cpl":
### Ab hier heißt "couplings" einfach nur genauso "ene" wie bei site-energy ###
    x, esp_tmp, ene, unused = shuffle_and_split(xyz_data, couplings, ntrain)
    energies = couplings
elif tgt=="site":
...
# Instantiate tuner
if tgt=="site" and esp_in_traindata:
    tuner = kt.Hyperband(
...
elif tgt=="site" and not esp_in_traindata:
    tuner = kt.Hyperband(
...
if create_origmodel:
    ...
    if tgt=="cpl":
        orig_model = build_model(subnets, output_spec, natoms*2, interatomic_dists,
                            norm=norm, loss=loss,
                            learning_rate = orig_learning_rate, print_summary=False,
                            make_subnet_predictions_accessible=False)
...
if tgt=="site" and esp_in_traindata:
    ...
    if create_origmodel:
        orig_hist = orig_model.fit([data["x_scaled"], data["esp"]], target, epochs=epochs,
                            validation_split=0.1, verbose=2, callbacks=[stop_early,lr_reduction])
elif (tgt=="site" and not esp_in_traindata) or tgt=="cpl":
